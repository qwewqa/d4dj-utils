...
def get_mix_data(songs: Sequence[MusicMaster]) -> List[MusicMixMaster]:
    ...
    for song in songs:
        if not (len(song.mix_info) == 3):
            raise ValueError(f'Song {song.name_description} does not have valid mix data.')
    return [songs[0].mix_info[1]] + [song.mix_info[2] for song in songs[1:-1]] + [songs[-1].mix_info[3]]