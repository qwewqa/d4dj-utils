...
class CardMaster(MasterAsset):
    ...
    def max_parameters_with_limit_break(self):
        return [
            self.rarity.total_limit_break_bonus + param for param in self.max_parameters
        ]
    ...
    def art_path(self, limit_break):
        return (
            self.assets.path
            / f"ondemand/card_chara/card_chara_{str(self.id).zfill(9)}_{1 if limit_break else 0}.jpg"
        )

    def icon_path(self, limit_break):
        return (
            self.assets.path
            / f"ondemand/card_icon/card_icon_{str(self.id).zfill(9)}_{1 if limit_break else 0}.jpg"
        )
    ...
    initial_card_cutoff = pytz.timezone("Asia/Tokyo").localize(
    ...
    def gacha(self) -> Optional[GachaMaster]:
        return next(
            (
                gacha
                for gacha in self.assets.gacha_master.values()
                if self in gacha.pick_up_cards and "★4" not in gacha.name
            ),
            None,
        )
